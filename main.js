...
async function startkirbotz() {
    ...
    const kirbotz = kirbotzConnect({
        ...
        browser: ['KennBot - MD','Safari','1.0.0'],
        auth: state,
        version
    })
    
console.log(color(figlet.textSync('KennBot', {
font: 'Standard',
...
cangtion = `Selamat Tinggal ${num.split("@")[0]}` 
...
let bton = [{
quickReplyButton: {
displayText: 'Selamat Tinggal Beban',
id: 'bye'
}
}]                  
...
kirbotz.ev.on('connection.update', async (update) => {
        ...
        let reason = new Boom(lastDisconnect?.error)?.output.statusCode
            ...
            else if (reason === DisconnectReason.connectionReplaced) { console.log("Connection Replaced, Another New Session Opened, Please Close Current Session First"); startkirbotz(); }
            else if (reason === DisconnectReason.loggedOut) { console.log(`Device Logged Out, Please Scan Again And Run.`); startkirbotz(); }
